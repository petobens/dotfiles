...
function M.delete_tool(kind)
    ...
    return {
        ...
        schema = {
            ...
            ['function'] = {
                name = tool_name,
                description = ('Delete a %s.'):format(kind_label),
                parameters = {
                    type = 'object',
                    properties = {
                        target = {
        ...
        output = {
            prompt = function(self, _)
                return ('Delete %s `%s`?'):format(kind_label, self.args.target)
            end,
            ...
            error = function(self, stderr, meta)
                helper.add_tool_error(
                    ...
                    self,
                    stderr,
                    ('%s deletion failed'):format(kind_label)
                )
            end,
        },
    }
