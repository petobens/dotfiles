...
vim.keymap.set({ 'n', 'i', 'x' }, '<down>', '<nop>', { desc = 'Disable <down>' })
vim.keymap.set({ 'n', 'i', 'x' }, '<left>', '<nop>', { desc = 'Disable <left>' })
vim.keymap.set({ 'n', 'i', 'x' }, '<right>', '<nop>', { desc = 'Disable <right>' })
vim.keymap.set({ 'n', 'i', 'x' }, '<up>', '<nop>', { desc = 'Disable <up>' })
...
vim.keymap.set('n', 'yy', function()
    local line = vim.api.nvim_win_get_cursor(0)[1]
    local remaining = vim.api.nvim_buf_line_count(0) - line + 1
    return 'mz0"' .. vim.v.register .. math.min(vim.v.count1, remaining) .. 'y$`z'
...
end, { expr = true, silent = false, desc = 'Expand to buffer directory' })
